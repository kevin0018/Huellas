...
export class ReminderController {
  async list(req: AuthenticatedRequest, res: Response): Promise<void> {
    await syncOwnerReminders(req.user.userId);
    const [owner, reminders] = await Promise.all([
      prisma.owner.findUniqueOrThrow({ where: { id: req.user.userId } }),
      prisma.reminder.findMany({ where: { pet: { owner_id: req.user.userId }, status: ReminderStatus.PENDING, OR: [{ snoozed_until: null }, { snoozed_until: { lte: new Date() } }] }, include: { pet: { select: { id: true, name: true } } }, orderBy: { due_at: 'asc' } }),
    ]);
  ...
  async action(req: AuthenticatedRequest, res: Response): Promise<void> {
    const reminder = await prisma.reminder.findFirst({ where: { id: Number(req.params.id), pet: { owner_id: req.user.userId } } });
    ...
    if (action === 'postpone') {
      ...
      await prisma.reminder.update({ where: { id: reminder.id }, data: { snoozed_until: new Date(now.getTime() + days * 86400000) } });
    } else if (action === 'complete') {
      await prisma.reminder.update({ where: { id: reminder.id }, data: { status: ReminderStatus.COMPLETED, completed_at: now, generated_action_at: now } });
    } else if (action === 'cancel') {
      await prisma.reminder.update({ where: { id: reminder.id }, data: { status: ReminderStatus.CANCELLED, cancelled_at: now } });
  ...
  async preferences(req: AuthenticatedRequest, res: Response): Promise<void> {
    ...
    const owner = await prisma.owner.update({ where: { id: req.user.userId }, data: { notifications_enabled: enabled, reminder_lead_days: leadDays } });
